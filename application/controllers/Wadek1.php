...
class Wadek1 extends CI_Controller {
	...
	public function dashboard() {
		$data = array(
			'suratList' => $this->M_surat->read_surat_where(array('status' => 2))->result(),
			'userDetail' => $this->M_basic->read_where('st_user',array('username' => $this->session->userdata('username')))->row(),
			'active' => 'Dashboard',
			'judul' => 'Dashboard',
			'msg' => $this->session->flashdata('msg')
		);
		$this->load->view('header/wadek1',$data);
		$this->load->view('wadek1/dashboard',$data);
	...
	public function suratList() {
		...
		$this->load->view('header/wadek1',$data);
		$this->load->view('wadek1/suratList',$data);
		$this->load->view('v_footer');
	}
	...
	public function suratDetail($id) {
		...
		$this->load->view('header/wadek1',$data);
		$this->load->view('wadek1/suratDetail',$data);
		$this->load->view('v_footer');
	}
	...
	public function doAddDisposisi() {
		...
		$data_update_surat = array('status' => 4);
		...
		redirect(base_url('Wadek1/suratDetail/').$this->input->post('id_surat'));
		} // end form validation
	} // end doAddDisposisi
